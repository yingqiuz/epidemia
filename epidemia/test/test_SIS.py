...
def test_si(sis_agents):
    region = np.random.randint(1, 10)
    edge = np.random.randint(1, 10)
    sis_agents.add_population(region=region, edge=edge)
    assert np.array_equal(
        sis_agents.region_population,
        np.zeros((2, N_REGIONS)) + region
    )
    assert np.array_equal(
        sis_agents.edge_population,
        (np.zeros((2, N_REGIONS, N_REGIONS)) + edge) * ADJ
    )
    # remove agents test
    sis_agents.remove_population(region=region, edge=edge)
    assert np.array_equal(
        sis_agents.region_population,
        np.zeros((2, N_REGIONS))
    )
    assert np.array_equal(
        sis_agents.edge_population,
        np.zeros((2, N_REGIONS, N_REGIONS))
    )
    ...
    prob_stay = np.random.rand(2, N_REGIONS)
    v = np.random.rand(N_REGIONS, N_REGIONS)
    sis_agents.set_spread_process(prob_stay=prob_stay, v=v)
    assert np.allclose(
        sis_agents.edge_to_region_weights,
        np.tile(
            DIST_INVERSE * v, [2, 1, 1]
        )
    )
    assert np.allclose(
        sis_agents.region_to_edge_weights,
        np.tile(
            ADJ_WEIGHTS / ADJ_WEIGHTS.sum(axis=1)[:, np.newaxis],
            [2, 1, 1]
        ) * (1 - prob_stay[:, :, np.newaxis]) +
        np.tile(
            np.eye(N_REGIONS), [2, 1, 1]
        ) * prob_stay[:, :, np.newaxis]
    )

    prob_stay = np.random.rand(N_REGIONS)
    sis_agents.set_spread_process(prob_stay=prob_stay, v=1)
    assert np.allclose(
        sis_agents.region_to_edge_weights,
        np.tile(
            ADJ_WEIGHTS / ADJ_WEIGHTS.sum(axis=1)[:, np.newaxis] *
    ...
    sis_agents.add_population(region=100, edge=100)
    sis_agents.spread_step()
    ...
    assert np.allclose(
        sis_agents.region_population[1],
        region_population
    )
    assert np.allclose(
        sis_agents.edge_population[1],
        edge_population
    )

    growth_rate = np.random.rand(N_REGIONS)
    sis_agents.region_growth_step(growth_rate=growth_rate)
    assert np.allclose(
        sis_agents.region_population[0],
        region_population + growth_rate * REGION_SIZE * 0.01
    )

    region_s = np.copy(sis_agents.region_population[0])
    region_i = np.copy(sis_agents.region_population[1])
    trans_rate = np.random.rand(N_REGIONS)
    sis_agents.region_transmission_step(trans_rate=trans_rate)
    assert np.allclose(
        sis_agents.region_population[0],
        region_s - region_s * (
    ...
    region_s = np.copy(sis_agents.region_population[0])
    region_i = np.copy(sis_agents.region_population[1])
    recovery_rate = np.random.rand(N_REGIONS)
    sis_agents.region_recovery_step(recovery_rate=recovery_rate)
    assert np.allclose(
        sis_agents.region_population[0],
        region_s + region_i * (
